...
КРИТИЧЕСКИ ВАЖНО для определения типа транзакции:
- "Доход" - ТОЛЬКО для ВХОДЯЩИХ переводов, зачислений, поступлений денег НА счет
- "Расход" - для ВСЕХ остальных операций: ИСХОДЯЩИХ переводов, списаний, оплат, платежей, покупок

Примеры:
- "Входящий перевод СБП" → Доход
- "Исходящий перевод СБП" → Расход
- "Оплата СБП QR" → Расход
- "Зачисление зарплаты" → Доход
- "Списание за услуги" → Расход

Для каждой транзакции определи:
- Тип: "Доход" ТОЛЬКО если деньги ПОСТУПАЮТ на счет, "Расход" для всех остальных случаев
